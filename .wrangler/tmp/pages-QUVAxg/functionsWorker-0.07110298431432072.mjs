...
async function onRequest({ request, env }) {
  ...
  const cookieState = cookieHeader.split(";").map((c) => c.trim()).find((c) => c.startsWith("oauth_state="));
  ...
  let user = await env.cloud_lens_data.prepare("SELECT * FROM users WHERE provider = ? AND provider_id = ?").bind("github", String(githubUser.id)).first();
  if (!user) {
    const result = await env.cloud_lens_data.prepare(
      `INSERT INTO users (provider, provider_id, email, username, avatar_url)
         VALUES (?, ?, ?, ?, ?)`
    ).bind("github", String(githubUser.id), email, githubUser.login, githubUser.avatar_url).run();
    user = {
  ...
  await env.cloud_lens_data.prepare(`INSERT INTO sessions (id, user_id, expires_at) VALUES (?, ?, ?)`).bind(sessionId, user.id, expiresAt.toISOString()).run();
...
// ../.wrangler/tmp/pages-QUVAxg/functionsRoutes-0.5860871691407716.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-4jQW0Z/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-4jQW0Z/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.07110298431432072.mjs.map
